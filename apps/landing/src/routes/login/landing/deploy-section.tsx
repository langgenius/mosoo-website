...
function CliMock(): ReactElement {
  return (
    <div className="bg-bg-sunken/60 w-full overflow-hidden rounded-[12px] p-4 font-mono text-[12px] leading-[1.85]">
      <p className="text-fg-3">{t("# deploy an agent from the CLI")}</p>
      ...
      <p className="text-fg-3">
        → run 4e9c1 · {t("1,284 docs")} · 12,480 tokens ·{" "}
        <span className="text-[#498C07]">{t("ok")}</span>
      </p>
    </div>
  );
}
...
const CONSOLE_ROWS = [
  { name: t("Support copilot"), state: "live" as const },
  { name: t("Contract reviewer"), state: "live" as const },
  { name: t("IT helpdesk"), state: "draft" as const },
];
  ...
  return (
    <div className="bg-bg-sunken/60 w-full overflow-hidden rounded-[12px]">
      <div className="border-border-soft flex items-center gap-1.5 border-b px-3 py-2">
        ...
        <span className="bg-ink-200 size-2 rounded-full" />
        <span className="bg-ink-200 size-2 rounded-full" />
        <span className="text-fg-3 ml-2 text-[11px] font-medium">{t("Agents")}</span>
      </div>
        ...
        {CONSOLE_ROWS.map((row) => (
          <li key={row.name} className="flex items-center gap-2.5 px-3 py-2">
            ...
            {row.state === "live" ? (
              ...
              <span className="text-fg-3 text-[10.5px] font-semibold">{t("Draft")}</span>
            )}
          </li>
        ))}
      </ul>
  ...
  return (
    <div className="flex w-full flex-col items-start gap-3">
      ...
      <p className="text-fg-3 font-mono text-[11px]">
        {t("provisions Workers · D1 · R2 · KV, no infra to wire up")}
      </p>
    </div>
  );
}
...
const CARDS = [
  {
    tag: t("CLI"),
    visual: <CliMock />,
    title: t("Run it from the terminal"),
    desc: t(
  ...
  },
  {
    tag: t("Web UI"),
    visual: <WebUiMock />,
    title: t("Or click through the console"),
    desc: t(
  ...
  },
  {
    ...
    title: t("Ship to your own Cloudflare"),
    desc: t(
      ...
      <div className="w-full">
        <Reveal className="max-w-[640px]">
          <h2 className="text-fg-1" style={sectionHeadingStyle}>
            {t("Manage everything from the CLI or the console.")}
          </h2>
