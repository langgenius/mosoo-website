...
const ZH_MESSAGES = {
  ...
  "Products that use mosoo as their Agent backend. Each case explains the product, the value mosoo provides, and the integration path.":
    "这些产品使用 mosoo 作为 Agent backend。每个案例都会说明产品、mosoo 提供的价值与集成路径。",
  ...
  "Codex Pet demonstrates how one published mosoo Agent can turn an uploaded avatar into a validated ZIP with all nine Codex pet animation states through a Cloudflare Worker.":
    "Codex Pet 展示了一个已发布的 mosoo Agent 如何通过 Cloudflare Worker，把上传的一张头像转化为包含全部九种 Codex 宠物动画状态、经校验的 ZIP。",
  "Read the case": "查看案例",
  ...
  "The Codex Pet case study shows how a workflow built in a coding IDE becomes a reusable mosoo-managed Agent exposed through an API — one uploaded avatar comes back as a validated ZIP with all nine Codex pet animation states.":
    "Codex Pet 案例展示了在 coding IDE 里搭好的工作流，如何变成一个由 mosoo 托管、通过 API 暴露的可复用 Agent——上传一张头像，返回包含全部九个 Codex 宠物动画状态、经校验的 ZIP。",
...
const JA_MESSAGES = {
  ...
  "Products that use mosoo as their Agent backend. Each case explains the product, the value mosoo provides, and the integration path.":
    "mosoo を Agent backend として使うプロダクト。各ケースでは、プロダクト、mosoo が提供する価値、統合経路を紹介します。",
  "A production fitness app where users log meals and workouts in natural language. One shared mosoo Agent turns every conversation into isolated, persistent user data.":
  ...
  "Codex Pet demonstrates how one published mosoo Agent can turn an uploaded avatar into a validated ZIP with all nine Codex pet animation states through a Cloudflare Worker.":
    "Codex Pet は、公開済みの mosoo Agent 1 つが Cloudflare Worker 経由で、アップロードされたアバターを 9 つの Codex ペットアニメーション状態を収めた検証済み ZIP に変換する方法を示します。",
  ...
  "The Codex Pet case study shows how a workflow built in a coding IDE becomes a reusable mosoo-managed Agent exposed through an API — one uploaded avatar comes back as a validated ZIP with all nine Codex pet animation states.":
    "Codex Pet のケーススタディは、coding IDE で組んだワークフローが mosoo 管理の再利用可能な Agent となり、API として公開される様子を示します。アップロードした 1 枚のアバターが、9 つの Codex ペットアニメーション状態を収めた検証済み ZIP になって返ります。",
