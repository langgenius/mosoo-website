...
而集成这件事本身，也不必由你手写。把 [mosoo.ai/coding-agents.md](https://mosoo.ai/coding-agents.md) 这份面向机器的说明文粘贴给你本地的 Claude Code 或 Codex，剩下的接入代码它会替你写完。你在本地用 coding agent 做出来的东西，由同一个 coding agent 把它接到云上——中间没有一堵需要你亲手翻越的墙。
